locals {
  ...
  hubs_list = flatten(
    [for namespace, values in var.eventhub_namespace_hubs :
  ...
  namespaces_reader = [for namespace, values in var.eventhub_namespace_hubs :
    namespace if lookup(var.eventhub_namespace_hubs[namespace], "reader", false)
  ]


  # Generate a list of namespaces to create shared access policies with sender right
  namespaces_sender = [for namespace, values in var.eventhub_namespace_hubs :
    namespace if lookup(var.eventhub_namespace_hubs[namespace], "sender", false)
  ]

  # Generate a list of namespaces to create shared access policies with manage right
  namespaces_manage = [for namespace, values in var.eventhub_namespace_hubs :
    namespace if lookup(var.eventhub_namespace_hubs[namespace], "manage", false)
  ]
